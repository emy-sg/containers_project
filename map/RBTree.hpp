...
template <class key_type, class value_type, class key_compare, class alloc>
class RBTree {
...
// -------------------------- Constructor ----------------------------
    RBTree(alloc map_alloc, key_compare comp) : _allocator(map_alloc), _compare(comp) {
...
// --------------------------- SearchByKey ------------------------
    ...
    Node*	searchByKey(const key_type& key) const{
	    Node* tmp = get_root();
	    ...
	    while (tmp->is_null() == false)
	    {
		    ...
		    if (_compare((tmp->value)->first, key) == 0 && _compare(key, (tmp->value)->first) == 0) //((tmp->value).first == key)
			{
		    ...
		    {
			    if (_compare((tmp->value)->first, key) == 1)  //((tmp->value).first < key)
				    tmp = tmp->r_child;
			    else
				    tmp = tmp->l_child;
		    }
	...
	Node*	searchLowerKey(const key_type& key) const 
	{
	    ...
	    while (tmp->is_null() == false)
	    {
		    if (_compare((tmp->value)->first, key) == 1)
				tmp = tmp->r_child;
			else if (_compare(key, (tmp->value)->first) == 1)
			{
				res = tmp;
				tmp = tmp->l_child;
			}
...
void	insertion_BST(Node* parent, Node* curr, Node* new_node) {
	...
	if (curr->is_null())
	{
		if (_compare(new_node->value->first, parent->value->first) == 1)//parent->value > new_node->value)
		{
	...
	{
		if (_compare(new_node->value->first, curr->value->first) == 1)//curr->value > new_node->value)
...
Node* insertion_RBTree(const value_type& val) {
	...
	else 	// case 2: Recursive
	{
		...
		// 4- Update begin of Tree
		if (_compare(val.first, _begin_node->value->first) == 1)//val < _begin_node->value)
			_begin_node = new_node;
	}
